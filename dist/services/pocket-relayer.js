...
class PocketRelayer {
    constructor(host, origin, userAgent, pocket, pocketConfiguration, cherryPicker, metricsRecorder, redis, databaseEncryptionKey, secretKey, relayPath, blockchainsRepository, checkDebug) {
        this.host = host;
        this.origin = origin;
    ...
    async sendRelay(rawData, application) {
        ...
        console.log("RELAYING APP " + application.id + " host: " + this.host + " req: " + data);
        ...
        if (relayResponse instanceof pocket_js_1.RelayResponse) {
            ...
            if ((blockchainEnforceResult) // Is this blockchain marked for result enforcement
                ...
                // Invalid
                console.log("INVALID " + application.id + " chain: " + blockchain + " req: " + JSON.stringify(data) + " res: " + relayResponse.payload + " node: " + relayResponse.proof.servicerPubKey);
        ...
        // Error
            ...
            console.log("ERROR " + application.id + " chain: " + blockchain + " req: " + JSON.stringify(data) + " res: " + relayResponse.message);
            await this.metricsRecorder.recordMetric({
                appPubKey: application.gatewayAAT.applicationPublicKey,
                blockchain,
    ...
    checkSecretKey(application) {
        ...
        if (application.gatewaySettings.secretKeyRequired // If the secret key is required by app's settings
            && // and 
                application.gatewaySettings.secretKey // the app's secret key is set
            && // and
                (!(this.secretKey) // the request doesn't contain a secret key
                    ...
                    || // or
                        ((this.secretKey.length === 32
                            &&
                                this.secretKey !== application.gatewaySettings.secretKey // the secret key does not match plaintext
                        )
                            || // or
                                (this.secretKey.length > 32
                                    &&
                                        this.secretKey !== decryptor.decrypt(application.gatewaySettings.secretKey) // does not match encrypted
                                )))) {
            return false;
        }
        return true;
