...
export class ChainChecker {
  ...
  async chainIDFilter(nodes: Node[], requestID: string, chainCheck: string, chainID: number, blockchain: string, applicationID: string, applicationPublicKey: string, pocket: Pocket, pocketAAT: PocketAAT, pocketConfiguration: Configuration): Promise<Node[]> {
    ...
    const CheckedNodesKey = chainID + '-' + crypto.createHash('sha256').update(JSON.stringify(nodes.sort((a,b) => (a.publicKey > b.publicKey) ? 1 : ((b.publicKey > a.publicKey) ? -1 : 0)), (k, v) => k != 'publicKey' ? v : undefined)).digest('hex');
    ...
    logger.log('info', 'CHAIN CHECK COMPLETE: ' + CheckedNodes.length + ' nodes on chain', {requestID: requestID, relayType: '', typeID: '', serviceNode: '', error: '', elapsedTime: ''});
    ...
    if (CheckedNodes.length < 5) {
      ...
      logger.log('info', 'CHAIN CHECK CHALLENGE: ' + JSON.stringify(consensusResponse), {requestID: requestID, relayType: '', typeID: '', serviceNode: '', error: '', elapsedTime: ''});
    }
    return CheckedNodes;
  }

  async getNodeChainLogs(nodes: Node[], requestID: string, chainCheck: string, blockchain: string, applicationID: string, applicationPublicKey: string, pocket: Pocket, pocketAAT: PocketAAT, pocketConfiguration: Configuration): Promise<NodeChainLog[]> {
    ...
    for (const node of nodes) {
      promiseStack.push(
        this.getNodeChainLog(node, requestID, chainCheck, blockchain, applicationID, applicationPublicKey, pocket, pocketAAT, pocketConfiguration)
      );
    }

    [ rawNodeChainLogs[0], rawNodeChainLogs[1], rawNodeChainLogs[2], rawNodeChainLogs[3], rawNodeChainLogs[4] ] = await Promise.all(promiseStack);
  ...
  async getNodeChainLog(node: Node, requestID: string, chainCheck: string, blockchain: string, applicationID: string, applicationPublicKey: string, pocket: Pocket, pocketAAT: PocketAAT, pocketConfiguration: Configuration): Promise<NodeChainLog> {
    
    logger.log('info', 'CHAIN CHECK START', {requestID: requestID, relayType: '', typeID: '', serviceNode: node.publicKey, error: '', elapsedTime: ''});
    ...
    if (
      ...
      const nodeChainLog = {node: node, chainID: parseInt(payload.result, 16)} as NodeChainLog;
      logger.log('info', 'CHAIN CHECK RESULT: ' + JSON.stringify(nodeChainLog), {requestID: requestID, relayType: '', typeID: '', serviceNode: node.publicKey, error: '', elapsedTime: ''});

      // Success
      return nodeChainLog;
    } 
    else if (relayResponse instanceof Error) {
      logger.log('error', 'CHAIN CHECK ERROR: ' + JSON.stringify(relayResponse), {requestID: requestID, relayType: '', typeID: '', serviceNode: node.publicKey, error: '', elapsedTime: ''});
      ...
      logger.log('error', 'CHAIN CHECK ERROR UNHANDLED: ' + JSON.stringify(relayResponse), {requestID: requestID, relayType: '', typeID: '', serviceNode: node.publicKey, error: '', elapsedTime: ''});
    }
    // Failed
